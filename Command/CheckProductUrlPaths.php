...
class CheckProductUrlPaths extends ConsoleCommand
{
    ...
    private const PROBLEM_DESCRIPTION = 'Product has a non-null url_path attribute, this is known to cause problems with url rewrites in Magento. It\'s adviced to remove this value from the database.';
    ...
    private function checkForNonEmptyUrlPathAttributeValues(OutputInterface $output): void
    {
        ...
        {
            // we need a left join when using the non-default store view and in especially for the case where storeId 0 doesn't have a value set for this attribute
    ...
    private function getProductsWithProblems(int $storeId, ProductCollection $collection): array
    {
        ...
        {
            $isOverridden = $this->attributeScopeOverriddenValueFactory->create()->containsValue(ProductInterface::class, $product, self::URL_PATH_ATTRIBUTE, $storeId);
